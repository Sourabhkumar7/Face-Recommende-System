...
if uploaded_image is not None:
    ...
    if save_uploaded_image(uploaded_image):
        ...
        #     st.image(filenames[index_pos],width=400,height=500)
        
       # Resize both images to have the same dimensions
# Resize both images to have the same dimensions
        resized_display_image = resize_image(display_image, 300, 400)
        predicted_image = Image.open(filenames[index_pos])
        resized_predicted_image = resize_image(predicted_image, 300, 400)
        ...
        with col2:
            ...
            st.image(resized_predicted_image)

            # Additional CSS for equal height and alignment
        st.markdown(
                """
                <style>
                .stImage > img {
